...
I would keep all states and logic in a container or custom hook
I would split UI into presentational components DropdownList and OptionItem would be stateless
All interactions would be handled by callbacks from the parent

I handle keyboard navigation using mousedown event
I prevent out-of-bounds using Math.min (for ArrowDown event) and Math.max (for ArrowUp event)
...
I would manage states for inputValue, isOpen and highlightedIndex.
...
To handle click outside, I would use a ref and a mousedown listener to detect when the click is outside the div.
