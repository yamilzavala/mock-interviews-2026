...
🎯 RESPUESTA NIVEL ENTREVISTA (corta)

I use a sliding window approach with a Set to track unique characters. When I find a duplicate, I shrink the window from the left until it becomes valid again. At each step, I update the maximum window size. This runs in O(n) time since each character is processed at most twice
